...
export const Route = createFileRoute("/projects")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "قائمة المشاريع التقنية التابعة للوزارة مع بحث وفلاتر متقدمة." },
    ],
  }),
});
      ...
      <Card className="p-4">
        ...
        <div className="mt-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="h-9 px-3 rounded-lg border border-border bg-background text-sm inline-flex items-center gap-2 text-muted-foreground hover:bg-accent">
      ...
      {filtered.length === 0 ? (
                  ...
                  return (
                    <tr key={p.id} className="border-t border-border hover:bg-accent/40 transition-colors">
                      <td className="px-4 py-3 font-medium">
                        <Link to="/projects/$id" params={{ id: String(p.id) }} className="hover:text-primary">
                          {p.name}
                        </Link>
                      </td>
                      ...
                      <td className="px-4 py-3 text-left">
                        <Link to="/projects/$id" params={{ id: String(p.id) }} className="text-primary text-xs font-semibold inline-flex items-center gap-1 hover:underline">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map((p) => {
            ...
            return (
              <Link
                to="/projects/$id" params={{ id: String(p.id) }} key={p.id}
