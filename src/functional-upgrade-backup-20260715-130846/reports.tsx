...
export const Route = createFileRoute("/reports")({
  ...
  head: () => ({ meta: [{ title: "التقارير — منصة إدارة المشاريع" }] }),
});
...
function ReportsPage() {
  ...
  return (
    ...
    >
      <Card className="p-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="relative md:col-span-2">
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="ابحث في التقارير" className="w-full h-11 pr-10 pl-3 rounded-lg border border-border bg-background text-sm" />
          </div>
      ...
      {filtered.length === 0 ? (
        <Card><EmptyState icon={<FileBarChart2 className="h-6 w-6" />} title="لا توجد تقارير مطابقة" /></Card>
      ) : (
