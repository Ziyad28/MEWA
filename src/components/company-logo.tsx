...
export function CompanyLogo({ domain, name, className = "h-12 w-20" }: { domain: string; name: string; className?: string }) {
  const [source, setSource] = useState(0);
  const sources = [LOGOS[domain], domain ? `https://${domain}/favicon.ico` : undefined].filter(Boolean) as string[];
  ...
  return (
    <span className={`${className} inline-flex items-center justify-center overflow-hidden ${domain === "sami.com.sa" ? "rounded-lg bg-[#123f35] p-2" : ""}`}>
      <img
        src={sources[source]}
        alt={`شعار ${name}`}
